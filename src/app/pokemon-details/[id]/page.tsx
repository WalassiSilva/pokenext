...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  const abilityDetails = pokemonAbilities.map((item, i) => {
    return (
      <details key={i} className="bg-gray-500 p-1">
  ...
  return (
    <div className="flex flex-col gap-5 *:capitalize relative">
      ...
      <div className="absolute top-1/2 -translate-y-1/2 left-3 flex justify-between border rounded-full p-1">
        <Link
          href={`/pokemon-details/${params.id === "1" ? 1025 : +params.id - 1}`}
        >
          <ChevronLeft />
        </Link></div>
        <div className="absolute top-1/2 -translate-y-1/2 right-3 flex justify-between border rounded-full p-1">
        <Link
          href={`/pokemon-details/${params.id === "1025" ? 1 : +params.id + 1}`}
        >
          <ChevronRight />
        </Link>
      </div>
